...
const breedingEvent = model<IBreedingEvent>('BreedingEvent', breedingEventSchema);

const addBreedingEvent = async (req: Request, res: Response, next: NextFunction) => {
    let id: Number = req.body.id;
    let date: Date = req.body.date;
    let male: String = req.body.male;
    let female: String = req.body.female;
...
const getBreedingEvent = async (req: Request, res: Response, next: NextFunction) => {
    let id: String = req.params.id;
    const result = await breedingEvent.findOne({id: id});
    return res.status(result ? 200 : 404).json(result);
};

const updateBreedingEvent = async (req: Request, res: Response, next: NextFunction) => {
    let id: string = req.params.id;
...
const deleteBreedingEvent = async (req: Request, res: Response, next: NextFunction) => {
    let id: string = req.params.id;
    const {acknowledged, deletedCount} = await breedingEvent.deleteOne({id: id});
...
export default { addBreedingEvent, getBreedingEvents, getBreedingEvent, updateBreedingEvent, deleteBreedingEvent }